...
*/
    ...
    {
        WebsStat sbuf;
        if (file_stat(cgiPath, &sbuf) != 0 || (sbuf.st_mode & S_IFREG) == 0) {
...
PUBLIC int websProcessCgiData(Webs *wp)
{
    ...
    if (file_write(wp->cgifd, wp->input.servp, (int) nbytes) != nbytes) {
...
PUBLIC void websCgiGatherOutput(Cgi *cgip)
{
    ...
    if ((file_stat(cgip->stdOut, &sbuf) == 0) && (sbuf.st_size > cgip->fplacemark)) {
        trace(5, "cgi: gather output");
        if ((fdout = file_open(cgip->stdOut, O_RDONLY | O_BINARY, 0444)) >= 0) {
            /*
             ...
             */
            wp = cgip->wp;
            file_lseek(fdout, cgip->fplacemark, SEEK_SET);
            while ((nbytes = file_read(fdout, buf, ME_GOAHEAD_LIMIT_BUFFER)) > 0) {
    ...
    for (cid = 0; cid < cgiMax; cid++) {
        if ((cgip = cgiList[cid]) != NULL) {
            ...
            if (checkCgi(cgip->handle) == 0) {
                 ...
                 */
                file_unlink(cgip->stdIn);
                file_unlink(cgip->stdOut);
                /*
