...
static bool parseIncoming(Webs *wp)
{
    ...
    *end = c;
    ...
    if (smatch(wp->method, "PUT")) {
        WebsStat    sbuf;
        wp->code = (file_stat(wp->filename, &sbuf) == 0 && sbuf.st_mode & S_IFDIR) ? HTTP_CODE_NO_CONTENT : HTTP_CODE_CREATED;
        wp->putname = websTempFile(ME_GOAHEAD_PUT_DIR, "put");
        if ((wp->putfd = file_open(wp->putname, O_BINARY | O_WRONLY | O_CREAT, 0644)) < 0) {
 ...
 */
static BOOLEAN flushChunkData(Webs *wp)
{
...
{
    /* home made time string */
    char buf[32];
    sprintf(buf, "%d", OSTimeGet());
    return sclone(buf);
#if 0
    WebsTime    now;
    struct tm   tm;
...
static char *makeSessionID(Webs *wp)
{
    ...
    assert(wp);
    fmt(idBuf, sizeof(idBuf), "%08x%08x%d", PTOI(wp) + PTOI(wp->url), (int) OSTimeGet(), nextSession++);
...
WebsSession *websGetSession(Webs *wp, int create)
{
    ...
    if (wp->session) {
        wp->session->expires = OSTimeGet() + wp->session->lifespan;
    }
    return wp->session;
}
