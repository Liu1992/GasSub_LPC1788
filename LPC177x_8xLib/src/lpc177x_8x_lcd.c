...
uint8_t FrameBuffer[C_GLCD_H_SIZE * C_GLCD_H_SIZE * 2];
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
 ...
 if ((x < LCD_XSIZE) && (y < LCD_YSIZE)) {
        *(volatile INT16U *)(LCD_VRAM_BASE_ADDR + 2*(xPhys + LCD_XSIZE * yPhys)) = (unsigned short)PixelIndex;
    }
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  /* Write into hardware ... Adapt to your system */
    ...
    if ((x < U_LCD_XSIZE) && (y < U_LCD_YSIZE)) {
        PixelIndex = *((volatile INT16U *)(LCD_VRAM_BASE_ADDR + 2*(xPhys + U_LCD_XSIZE * yPhys))); 
		return PixelIndex;
    } 
	return 0;
}
