...
#define MENU_FONT_DEFAULT         &GUI_FontHZ16
#endif
...
#define MENU_BKCOLOR1_DEFAULT     0x000098
#endif
...
#define MENU_BKCOLOR3_DEFAULT     0x000098
#endif
...
unsigned int  _SellecNextItem(  MENU_Obj*   pObj)
{
    int Index;
    MENU_ITEM* pItem;
    Index=pObj->Sel;
    ...
    while(Index<pObj->ItemArray.NumItems);
    if(Index>=pObj->ItemArray.NumItems)
    {
        Index=0;
    };
    return(Index);
...
unsigned int  _SellecPreviousItem(  MENU_Obj*   pObj)
{
    int Index;
    MENU_ITEM* pItem;
    Index=pObj->Sel;
    ...
    if(Index<0)
    {
        Index=pObj->ItemArray.NumItems-1;
    };
    return(Index);
}
...
*       _MENU_AddKey
*/
void _MENU_AddKey( MENU_Handle hObj,  MENU_Obj* pObj,  int Key)
{
    MENU_Handle hSub;
    MENU_Handle hOwner;
    int Index;
    ...
    case GUI_KEY_TAB:
        ...
        if (pObj->Flags & MENU_SF_VERTICAL) 	//如果是垂直菜单
        {
            pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray,pObj->Sel);
            if(pItem->hSubmenu) //如果有下级菜单
            {
                WM_SetFocus(pItem->hSubmenu);
                hSub=pItem->hSubmenu;

                pSub=MENU_H2P(pItem->hSubmenu);//?????????????????????????????????????
                
				Index=_SellecNextItem(pSub);
                pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pSub->ItemArray,Index);
                _DeselectItem(pItem->hSubmenu, pSub);
                _SelectItem(pItem->hSubmenu, pSub, Index);
                if(pItem->hSubmenu) //如果有下级菜单
                {
                    _DeactivateMenu(hSub,  pSub);
                    _ActivateMenu(hSub,  pSub, pSub->Sel);
                };
            }
            else//如果有无下级菜单
            {
                hOwner=pObj->hOwner;							 //找到它的父菜单
                pOwner=MENU_H2P(hOwner);
                _DeactivateMenu(hOwner,pOwner);					 //取消对其父菜单的激活状态
                Index=_SellecNextItem(pOwner);					 //选中它父菜单的下一个菜单
                pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pOwner->ItemArray,Index);
                if(pItem->hSubmenu) //如果其父菜单有下级菜单
                {
                    _DeactivateMenu(hOwner,  pOwner);
                    _ActivateMenu(hOwner,  pOwner, Index);
                }
                else//没有下级，只打开选项
                {
                    _DeselectItem(hOwner, pOwner);
                    _SelectItem(hOwner, pOwner, Index);
    ...
    case GUI_KEY_DOWN:
        ...
        {
            pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray,pObj->Sel);
            if( pItem && pItem->hSubmenu ) //如果有下级菜单,增加首先判断pItem是否为空-2014.4.13
            {
                WM_SetFocus(pItem->hSubmenu);
                hSub=pItem->hSubmenu;
                pSub=MENU_H2P(pItem->hSubmenu);//?????????????????????????????????????
        ...
        break;
    case GUI_KEY_UP:    //加 键当做返回键使用-2014.4.13
   /*
	 	hOwner=pObj->hOwner;
        pOwner=MENU_H2P(hOwner);
	...
	*/
	...
	/*
		...
		if (pObj->Flags & MENU_SF_VERTICAL) //如果是垂直菜单
        {
            Index=_SellecPreviousItem(pObj);
            pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray,Index);
        ...
        else if ( pObj->Flags & MENU_SF_HORIZONTAL )//如果是水平菜单
        {
            pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray,pObj->Sel);
            if(pItem->hSubmenu) //如果有下级菜单
            {
                ...
                hSub=pItem->hSubmenu;
                pSub=MENU_H2P(pItem->hSubmenu);//?????????????????????????????????????
                Index=_SellecPreviousItem(pSub);
        ...
        break;
    case GUI_KEY_ENTER:
        _ActivateItem(hObj,pObj,pObj->Sel);
