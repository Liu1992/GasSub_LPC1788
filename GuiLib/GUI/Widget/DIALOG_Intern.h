...
struct GUI_WIDGET_CREATE_INFO_struct {
  ...
  const char* pName;                     /* 文本（不是所有的控件都使用Text ... Not used on all widgets */
  I16 Id;                                /* 控件的窗口 ID ;ID ... should be unique in a dialog */
  I16 x0, y0, xSize, ySize;              /* 控件的大小和位置 Define position and size */
  U16 Flags;                             /* 控件专用标识（或0）Widget specific create flags (opt.) */
  I32 Para;                              /* 控件专用参数（或0）Widget specific parameter (opt.) */ 
};
